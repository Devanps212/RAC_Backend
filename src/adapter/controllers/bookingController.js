...
const bookingController = (bookingInterface, bookingDBRepository, bookingModel, carInterface, carRepository, carModel, userModel, userInterface, userRepository, paymentInterface, paymentServices, couponInterface, couponRepository, couponModel) => {
    ...
    const filteringCarsBooking = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
        const { pickupLocation, dropOffLocation, startDate, endDate, pickupTime, dropOffTime } = req.query;
        const finDcars = yield (0, car_1.findCar)('all', carService);
        const Bookings = yield (0, booking_1.findBooking)('all', bookingService);
        if (finDcars) {
            const ownerId = finDcars.map((car) => car.addedById);
            const findUsersPromises = ownerId.map(id => (0, userAuth_1.findUser)(id, userService));
        }
    }));
    const findBookings = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
        const data = req.query.bookingData;
        if (typeof data === 'string') {
            const findingBookings = yield (0, booking_1.findBooking)(data, bookingService);
            res.json({
    ...
    const bookingPaymentUI = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
        const { dataString, carId, userId } = req.body;
        const bookingDetail = JSON.parse(dataString);
        const carData = yield (0, car_1.findCar)(carId, carService);
        const payment = yield (0, booking_1.bookingPayment)(bookingDetail, carData, userId, paymentService);
        req.session.userData = payment;
        res.json({
            sessionId: payment
        });
    }));
    const bookingCompletion = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (typeof val === 'string' && typeof bookingDetail === 'string' && typeof session_id === 'string') {
            ...
            const carDetails = yield (0, car_1.findCar)(carId, carService);
            const sesssionVerification = yield (0, booking_1.stripePaymentVeification)(session_id, paymentService);
            if (sesssionVerification) {
                paymentDetail.transactionId = session_id;
            }
            const bookingId = paymentDetail.bookingDetails.bookingId || '';
            console.log(bookingId);
            const booking = yield (0, booking_1.findBooking)(bookingId, bookingService);
            if (booking !== null) {
                ...
                if (bookingData.amount && bookingData.startDate && bookingData.endDate) {
                    ...
                    const updateBooking = yield (0, booking_1.BookingUpdater)(data, bookingService);
                ...
                const bookingCreation = yield (0, booking_1.createBooking)(paymentDetail, carDetails, bookingService);
                if (bookingCreation !== null) {
                    const price = bookingCreation.transaction.amount;
                    const AllCoupons = yield (0, coupon_1.findAllCoupon)(couponService);
                    if (AllCoupons && AllCoupons.length > 0) {
                        ...
                        if (matchedCoupons.length > 0) {
                            ...
                            const userUpdate = yield (0, user_1.updateUser)(data, userService);
                            if (userUpdate === null) {
                                res.json({
                    ...
                    const statusUpdateCar = yield (0, car_1.updateCar)(carId, update, carService);
                    if (statusUpdateCar) {
    ...
    const bookingFindingBasedOnRole = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
        const { bookingData } = req.body;
        const findBooking = yield (0, booking_1.bookingBasedOnRole)(bookingData, bookingService);
        res.json({
            data: findBooking
        });
    }));
    const bookingUpdater = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
        var _a, _b;
        const { data, purpose } = req.body;
        const bookingDetail = data;
        if (bookingDetail && bookingDetail._id) {
            const booking = yield (0, booking_1.findBooking)(bookingDetail._id, bookingService);
            if (!booking || (booking && 'message' in booking)) {
                res.status(404).json({
                    message: (booking === null || booking === void 0 ? void 0 : booking.message) || "Booking not found",
                    status: 'failed'
                });
                return;
            }
            ...
            const update = yield (0, booking_1.BookingUpdater)(bookingDetail, bookingService);
            if (!update || (update && 'message' in update)) {
                res.status(404).json({
                    message: (update === null || update === void 0 ? void 0 : update.message) || "Failed to update booking",
                    status: 'failed'
                });
                return;
            }
            if (update.status === 'Cancelled' && purpose === 'refund') {
                const refund = yield (0, booking_1.stripeRefund)(update, paymentService);
                const refundData = {
                ...
                const userRefundUpdate = yield (0, user_1.updateUser)(data, userService);
                if (!userRefundUpdate) {
                    res.json({ error: "refund Failed", status: "failed" });
                }
                if (typeof booking.carId === 'object') {
                    refund.bookingDetail = {
                        itemName: (_a = booking.carId.name) !== null && _a !== void 0 ? _a : '',
                        thumbnail: (_b = booking.carId.thumbnailImg) !== null && _b !== void 0 ? _b : ''
                    };
                }
                res.json({
    ...
    const bookingRescheduler = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (datas && typeof datas.carId === 'object') {
            ...
            if (carId && typeof carId === 'string') {
                ...
                const payment = yield (0, booking_1.bookingReschedule)(datas, carData, userId, paymentService);
                res.json({
                    sessionId: payment
                });
                return;
    ...
    const carReportHandler = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
        const { data, bookingId } = req.body;
        const carDetails = data;
        const carResult = yield (0, car_1.findCar)(String(carDetails._id), carService);
        ...
        const bookingResult = yield (0, booking_1.findBooking)(bookingId, bookingService);
        ...
        if (bookingResult.status === 'Cancelled' || bookingResult.status === 'Completed') {
            ...
            const data = yield (0, car_1.updateCar)(String(carResult._id), carResult, carService);
            yield (0, booking_1.BookingUpdater)(bookingResult, bookingService);
    ...
    const topBookedCars = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
        const bookingResult = yield (0, booking_1.findBooking)('all', bookingService);
        const carResult = yield (0, car_1.findCar)('all', carService);
        if (Array.isArray(carResult) && Array.isArray(bookingResult)) {
            const carBookingCount = {};
            bookingResult.forEach((booking) => {
                var _a;
                const carId = typeof booking.carId === 'string' ? booking.carId : (_a = booking.carId._id) === null || _a === void 0 ? void 0 : _a.toString();
                if (carId) {
            ...
            const sortedCars = Object.entries(carBookingCount)
                ...
                .slice(0, 4);
            const topCars = sortedCars.map(({ carId }) => carResult.find(car => { var _a; return ((_a = car._id) === null || _a === void 0 ? void 0 : _a.toString()) === carId; })).filter(car => car);
            res.status(200).json({
                car: topCars,
                status: "success"
            });
    ...
    const paginationBooking = (0, express_async_handler_1.default)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const bookingPage = yield (0, booking_1.bookingPagination)(parsedData, parseInt(page), parseInt(limit), bookingService);
        res.status(200)
            .json({
            message: "success",
